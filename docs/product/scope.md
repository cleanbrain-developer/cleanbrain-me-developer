...
- `HttpRelayHubAdapter` and any real RelayHub backend call — V1 is mock-only (see `docs/architecture/overview.md`, "Adapter pattern").
...
- Whether/when `HttpRelayHubAdapter` becomes real, and which `relayhub-java` API surface it would call — deferred to a dedicated ADR when that requirement is real (see `docs/product/goals.md`, "Long-term direction").
